...
void CurvedLine::SetPoints(sf::Vector2f f, sf::Vector2f t, bool should_update) {
    from = f;
    to = t;
...
void CurvedLine::SetRadius(float r, bool should_update) {
    radius = r;

    if (should_update) Build();
}

void CurvedLine::SetSmoothness(int s, bool should_update) {
    smoothness = s;

    if (should_update) Build();
}

void CurvedLine::CurveAt(float c, bool should_update) {
    curving_point = c;

    if (should_update) Build();
}

void CurvedLine::SetColor(sf::Color c, bool should_update) {
    color = c;
...
void CurvedLine::draw(sf::RenderTarget& rt, sf::RenderStates rs) const {
